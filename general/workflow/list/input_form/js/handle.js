...
function showInstrustDiv() {
    ...
    jQuery('#myModal_intrust').find('.modal-body').load('/module/workflow/engine/data/work_run_intrust.php?run_id='+run_id+'&flow_id='+flow_id+'&flow_prcs='+flow_prcs+'&prcs_id='+prcs_id+'&prcs_key_id='+prcs_key_id, function() {
        ...
        jQuery("button[btntype='work_run_intrust']").click(function() { //普通操作
            ...
            if (delegate_type === '2') { //自由委托  btn-success
                var module_id = '', 
...
//            child_window.CheckForm(7);
    ...
    jQuery('#work_run_submit').on('click', function() { // 工作转交下一步的确认操作
        ...
        var RUN_PRCS_NAME = jQuery("#RUN_PRCS_NAME").val();
        // var over_sign_length = jQuery('#workPrcsData li.workflow-node').length;
        // var over_sign = jQuery('#workPrcsData li.workflow-node').eq(over_sign_length-1).attr('over_sign');
        // if(!jQuery('#workPrcsData li.workflow-node').eq(over_sign_length-1).hasClass('active'))
        // {
            // over_sign = 0;
        // }
        var next_prcs_id = "";
        if(sync_deal != 0)//允许并发,强制并发
        {
            jQuery("li[id^='next_prcs_'][class*='active']").each(function(i,v){
                var check_result = jQuery(v).attr("check_result");
                var before_prcs_name = jQuery(v).attr("before_prcs_name");
                if(check_result == 1 && before_prcs_name != "")
                {
        ...
        {
            var check_is_myself = jQuery("li[id^='next_prcs_'][class*='active']").attr("check_is_myself");
            if(check_is_myself == 1)
            {
                next_prcs_id = jQuery("li[id^='next_prcs_'][class*='active']").attr("id").substr(10);
            }
    ...
    jQuery(document).on('click', '#all_check', function() {
        ...
        if (!this.className.match(/active/)) {
            jQuery("li[id^='next_prcs_']").each(function(i, v) {
                if ((!jQuery(v).hasClass('active')) && jQuery(v).attr('id') != 'next_prcs_0' && sync_deal == 1 && jQuery(v).attr('id').split('_')[2] != flow_prcs && jQuery(v).attr('check_result') == 1) {
...
function addPrcsJsonDatas(data) {
    ...
    jQuery(".workflow-node").click(function() {
        ...
        var next_prcs_name_str = jQuery(this).attr('next_prcs_name');
        var before_prcs_name = jQuery(this).attr('before_prcs_name');
        var check_result_ = jQuery(this).attr('check_result');
        ...
        if (sync_deal == 0) {
            ...
            if ($this.hasClass('active')) {
                ...
                if(jQuery("#work-free-prcs-block-"+ flow_prcs +" ").css("display") == "none" && $this.attr('data-type') == 'flexible'){
                    ...
                    if(before_prcs_name == "")
                    {
                        jQuery('#workPrcsData li.workflow-node').each(function(i,v){
                            var before_prcs_name_arr = jQuery(v).attr('before_prcs_name');
                            if(before_prcs_name_arr != ""){
                                var before_prcs_name_arrs = before_prcs_name_arr.split(",");
                                for(var m=0;m<before_prcs_name_arrs.length;m++)
                                {
                                    if(before_prcs_name_arrs[m] == next_prcs_name_str)
                                    {
                    ...
                    {
                        var before_prcs_name_arr = before_prcs_name.split(",");
                        for(var n=0;n<before_prcs_name_arr.length;n++)
                        {
                            jQuery('#workPrcsData li.workflow-node').each(function(i,v){
                                var next_prcs_name_str = jQuery(v).attr("next_prcs_name");
                                if(before_prcs_name_arr[n] == next_prcs_name_str)
                                {
                                    if(jQuery(v).hasClass('active'))
                                    {
                                        jQuery(v).trigger('click');
                ...
                var prcsBlockData = [{
                        ...
                        'gather_node_have_flag': $this.attr('gather_node_have_flag'),
                        'next_prcs_name': $this.attr('next_prcs_name'),
                        'check_result': $this.attr('check_result'),
                    ...
                    if(prcs_info_arr[2] === "0" && typeof $this.attr("prcs_back") != 'undefined'){
                        ...
                        if(jQuery("#work-free-prcs-block-"+ flow_prcs +" ").css("display") == "none" && $this.attr('data-type') == 'flexible'){
                            ...
                            if(check_result_ == 0)
                            {
                                if(before_prcs_name == ""){
                                    jQuery('#workPrcsData li.workflow-node').each(function(i,v){
                                        var before_prcs_name_ = jQuery(v).attr('before_prcs_name');
                                        if(before_prcs_name_ != "")
                                        {
                                            var before_prcs_name_arr = before_prcs_name_.split(',');
                                            for(var j=0;j<before_prcs_name_arr.length;j++)
                                            {
                                                if(before_prcs_name_arr[j] == next_prcs_name_str)
                                                {
                                                    $this.attr('check_result','1');
                                                    $this.addClass('active');
                                                    jQuery(v).attr('check_result','0');
                                ...
                                }else
                                {
                                    ...
                                    {
                                        jQuery('#workPrcsData li.workflow-node').each(function(i,v){
                                            var next_prcs_name_ = jQuery(v).attr('next_prcs_name');
                                            if(next_prcs_name_ == before_prcs_name_arr[j])
                                            {
                                                jQuery(v).removeClass('active');
                                                jQuery(v).attr('check_result','0');
                                                if(!$this.hasClass('active'))
                            ...
                            // jQuery('#workPrcsData li.workflow-node').each(function(i,v){
                                // var check_result_ = jQuery(v).attr('check_result');
                                // if()
                            // });
                            jQuery(".workflow-node[id!='next_prcs_0'][check_result !=0][data-type!='flexible'][condition!='false']").addClass('active');  // 选中条件不为false的步骤节点
                            ...
                            var $li_obj = jQuery(".workflow-node[id!='next_prcs_0'][check_result!=0][data-type!='flexible'][condition!='false']");
                        }
                    }
                    jQuery('div[data_type="next"]').show();
    ...
    {
        var check_result = prcs_data_arr[i].check_result;
        var prcs_num_str = prcs_data_arr[i].prcs_num;
        var before_prcs_name_str = prcs_data_arr[i].before_prcs_name;
        var prcs_num = parseInt(prcs_num_str);
        if ((sync_deal == 1 || sync_deal == 2) && check_result==1) {
            if(before_prcs_name_str != "")
                jQuery('li[id^="next_prcs_'+prcs_num+'"][before_prcs_name^="'+before_prcs_name_str+'"]').trigger('click');
            else
            ...
            if(prcs_data_arr.length > 1)
            {
                ...
                if(prcs_data_arr[1].prcs_num != 0)
                {
                    jQuery('#next_prcs_' + prcs_data_arr[1].prcs_num).click();
                }else
                {
                    jQuery('#next_prcs_' + data.defaultNextPrcs).click();
                }
    ...
    // jQuery(".workflow-node").find('span').hover(function(){
            // jQuery(".workflow-node").find('span').tooltip({
                // track:true
            // });
        // },jQuery('#next_prcs_' + data.defaultNextPrcs).click();
        // function(){
