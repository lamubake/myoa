...
function calc_DATA(rule, reminder, type){
    ...
    var myvalue= "";
    if(type == "date"){
        myvalue=eval("calc_date("+rule+")");
    }else if(type == "day"){
        myvalue=eval("calc_day("+rule+")");
    }else if(type == "hour"){
        myvalue=eval("calc_hour("+rule+")");
    }
    var prec = reminder || 4;
    var vPrec;
    if(!prec){
...
define('TCalcCtrl', function(require, exports, module){
    ...
    var TCalcCtrl = Base.extend({
            ...
            $.each(arr, function(m, n){
			    $.each(instances, function(k, v){
					...
					if(v.title == n){
						...
						var re = new RegExp(n,"ig");
                        var calv = 0;
                        if(v.$config.secret || v.$config.ishidden || v.$config.unCalc){
        ...
        setValue: function(rule, reminder){
            var self = this;
            var day_pattern = /^DAY\((.*?)\)$/;
            var date_pattern = /^DATE\((.*?)\)$/;
            var hour_pattern = /^HOUR\((.*?)\)$/;
