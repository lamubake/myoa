jQuery(document).ready(function(){
    var sizeArr = getLogGridTableSize();
     ...
     $.getJSON("/general/approve_center/get_flow_list.inc.php",{"FLOW_ID":MENU_FLOW_ID,"action":1,"root":true},function(jsonData){
        $("#FLOW_ID").html("");
    ...
    jQuery(document).on("click", '#queryBtn', function(){
        ...
        var url = 'data/getdata.php?action=monitor_work&flow_id='+FLOW_ID+'&user_type='+USER_TYPE+'&to_id='+TO_ID+'&run_id='+RUN_ID+'&run_name='+RUN_NAME+'&pageType='+pageType+'&work_level='+WORK_LEVEL;
...
//    });
    ...
    jQuery.getJSON("data/getcolumns.php", {
        ...
        function(columns){
            ...
            //alert(page);
            var url = 'data/getdata.php?action=monitor_work&flow_id='+FLOW_ID+'&user_type='+USER_TYPE+'&to_id='+encodeURI(TO_ID)+'&run_id='+RUN_ID+'&run_name='+RUN_NAME+'&pageType='+pageType+'&work_level='+WORK_LEVEL;
    ...
    jQuery("#gridTable").find('tr').each(function(){
        ...
        if( I != 1)
        {
            ...
            if(key == flag)
            {
                if(K == 0)
                {
                    var tdhtml = jQuery(this).find("td").eq(0).find("a").html();
                    if(typeof(tdhtml)!='undefined' && tdhtml.indexOf('icon-plus') !== -1)
                    {
                        jQuery(this).find("td").eq(0).find("a").html("<i class=\"icon-minus\"></i>");
                        collapse_show(flag);
                    }else{
                        jQuery(this).find("td").eq(0).find("a").html("<i class=\"icon-plus\"></i>");
                        collapse_hide(flag);
                    }
                }
                K++;
