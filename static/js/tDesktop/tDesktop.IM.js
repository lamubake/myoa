                ...
                success: function(data){
                    ...
                    for(var i=0; i< array.length; i++)
                    {
                        ...
                        if(window.localStorage || typeof window.localStorage != 'undefined'){
                            ...
                            data.send_time_stamp = array[i].send_time_stamp; 
            ...
            $.ajax({
                ...
                success: function(d){
                    ...
                    if(window.localStorage || typeof window.localStorage != 'undefined'){
                        var d1 = new Object;
                        d1.sms_id = d.sms_id;
                        d1.send_time = date.toTimeString().substr(0,5); 
                        d1.send_time_stamp = parseInt(date.getTime()/1000);
                        d1.avatar = $(".chatuser.active img").attr("src");
                        d1.from_id = user_id;
                        d1.to_id = loginUser.uid;
                        d1.from_name = user_name;
                        d1.from_type_name = "微讯网页版";
                        d1.content = encodeURIComponent(content);
                        d1.from_name = encodeURIComponent(d1.from_name);
                        var str = JSON.stringify(d1);
                        str = Base64.encode(str);
                        localStorage.setItem("tdesktop_"+d1.from_id, str);
                    }
            ...
            render: function(data){
                ...
                $.each(data,function(index,item){
                    ...
                    if(item.avatar.indexOf(".")!=-1 && item.avatar.indexOf('/')==-1){
                        ...
                        item.avatar = static_server + '/static/images/avatar/avatar_'+item.avatar+'.jpg';  
                    }
                })
        ...
        fixAvatar: function(uid){
            ...
            $.ajax({
                ...
                success: function(d){
                    ...
                    $('#recentlist [fromid="'+uid+'"]').find('img').attr('src',avatar);
                    if(window.localStorage || typeof window.localStorage != 'undefined'){
                        for(var i=0; i<localStorage.length; i++ ){
                            ...
                            if(key == ("tdesktop_"+uid)){
                                ...
                                try{
                                    ...
                                    data.avatar = avatar;
                                    str = Base64.encode(JSON.stringify(data));
            ...
            if(window.localStorage || typeof window.localStorage != 'undefined'){
                for(var i=0; i<localStorage.length; i++ ){
                    ...
                    if(key.indexOf("tdesktop_") != -1){
                        ...
                        data.time_stamp = data.send_time_stamp? formatTime(data.send_time_stamp):data.send_time;
                        if( $("a[fromid='"+data.from_id+"']").length > 0 ){
                            $("a[fromid='"+data.from_id+"']").find('img').attr('src',data.avatar);
                            $("a[fromid='"+data.from_id+"']").find('.rec_time').text(data.time_stamp);
