(function($){
	...
	$.userselect = function(el, settings) {
        ...
        // localstorage
        me.store = {
            _data: JSON.parse(localStorage.getItem(settings.storageName)),
            set: function (items) {
        ...
        me.recentSelected = (function(){
            try{
                //定义一个数组存下最近搜索并选择过的人
                var recentUser = JSON.parse(localStorage["recent_user"] ? localStorage["recent_user"] : null) || [];
                //储存已选择的用户
	...
	$.fn.userselect.defaults = {
        ...
        tpl : {
            ...
            wrapper : '<section class="ui-container userselect-ui-container"><footer class="ui-footer ui-footer-btn"><div class="ui-footer ui-footer-stable ui-btn-group ui-border-t"><button node-type="confirm" class="ui-btn-lg ui-btn-primary">确认</button><button node-type="cancel" class="ui-btn-lg">取消</button> </div></footer><section class="ui-container"><div class="ui-tab"><ul class="ui-tab-nav ui-border-b"><li>已选</li><li>搜索</li></ul><ul class="ui-tab-content" style="width:200%;"><li><ul class="ui-list ui-border-tb"></ul></li><li><div class="ui-searchbar-wrap ui-border-b focus"><div class="ui-searchbar ui-border-radius"><i class="ui-icon-search"></i><div class="ui-searchbar-text">用户姓名</div><div class="ui-searchbar-input"><input value="" type="text" placeholder="用户姓名" autocapitalize="off"></div><i class="ui-icon-close"></i></div></div><div class="ui-label-list"></div><ul class="ui-list ui-border-tb"></ul></li></ul></div></section></section>',
            select: '<li class="ui-border-t" data-user-uid="{{uid}}" data-user-id="{{id}}"><div class="ui-list-thumb-s"><i class="ui-icon-success"></i></div><div class="ui-list-info"><h4 class="ui-nowrap">{{name}}<span class="date">（{{priv}}）</span></h4><p><span>部门：{{dept}}</span></p></div></li>',
            search: '<li class="ui-border-t unselected" data-user-uid="{{uid}}" data-user-id="{{id}}" data-user-name="{{name}}" data-dept-name="{{dept}}" data-priv-name="{{priv}}"><div class="ui-list-thumb-s"><i class="ui-icon-success"></i></div><div class="ui-list-info"><h4 class="ui-nowrap">{{name}}<span class="date">（{{priv}}）</span></h4><p><span>部门：{{dept}}</span></p></div></li>',
