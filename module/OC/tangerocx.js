...
function AddSignFromLocal(key)
{
   if(TANGER_OCX_bDocOpen)
   {
      /*if(TANGER_OCX_OBJ.IsNTKOSecSignInstalled())
         TANGER_OCX_OBJ.AddSecSignFromLocal(TANGER_OCX_user, "", true, 0, 0, 1, 2, false, false, true, false, "11111111", false, true);
      else*/
...
{
   if(TANGER_OCX_OBJ.IsNTKOSecSignInstalled())
      TANGER_OCX_OBJ.AddSecSignFromEkey(TANGER_OCX_user, 0, 0, 1,2,false,false,true,false,"",-1,false,true);
}
//开始手写签名
...
function TANGER_OCX_AddDocHeaderExec()
{
	try{
		...
		if(window.opener)
		{
		    ...
		    if(parentUrl.indexOf("workflow/list/input_form")>0)
		    {
                addFlowData();
		    }
		}
...
function addFlowData()
{
    ...
    for(var i=1;i<=bookmarks.Count;i++)
    {
        ...
        if(bookmark_name != "zhengwen")
        {
            ...
            if(typeof bookmark_value == 'undefined') continue;
...
function TANGER_OCX_SetInfo()
{
	...
	try
	{
   	...
   	{
   	   TANGER_OCX_OBJ.AddDocTypePlugin(".pdf","PDF.NtkoDocument","4.0.0.9","./ntkopdfdoc.cab",51,true);
      	TANGER_OCX_OBJ.AddDocTypePlugin(".tif","TIF.NtkoDocument","4.0.0.9","./ntkopdfdoc.cab",52);
      	TANGER_OCX_OBJ.AddDocTypePlugin(".tiff","TIF.NtkoDocument","4.0.0.9","/ntkopdfdoc.cab",52);
      }
