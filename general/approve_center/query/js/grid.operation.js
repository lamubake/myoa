jQuery(document).ready(function(){
    ...
    jQuery.getJSON("/general/approve_center/get_flow_list.inc.php",{"FLOW_ID":MENU_FLOW_ID,"action":4,"root":true},function(jsonData){
        jQuery("#FLOW_ID").html("");
    ...
    if(!MENU_FLOW_ID){
        jQuery.getJSON("data/getcolumns.php", {
            ...
            function(columns){
                ...
                var url = 'data/getdata.php?action=list_allwork&flow_id='+FLOW_ID+'&begin_time='+BEGIN_TIME+'&end_time='+END_TIME+'&run_id='+RUN_ID+'&flow_status='+FLOW_STATUS+'&run_name='+RUN_NAME+'&flow_query_type='+FLOW_QUERY_TYPE+'&to_id='+TO_ID+'&work_level='+WORK_LEVEL;
...
function enforcement_end_run()
{
    ...
    {
        jQuery.get("/module/approve_center/engine/verify_end_work.php",{"RUN_ID_STR":runIdStr,"FLOW_ID":flowIdStr,"BEGIN_DEPT":beginDeptStr},function(data)
        {    
...
function delete_run_sel()
{
    ...
    if(window.confirm(msg))
    {
        jQuery.get("/module/approve_center/engine/delete_work.php",{"action":"del_work","run_id_str":runIdStr,"flow_id":flowIdStr,"begin_dept":beginDeptStr},function(data)
        {    
            jQuery("#gridTable").jqGrid('setGridParam', { serializeGridData : function(e){ e.connstatus = 1;  return e;} }).trigger('reloadGrid');
            if(data != "ALL"){
