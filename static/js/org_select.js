...
function get_items(url, block, item_id)
{
   ...
   {
      var println = url.split('?');
      var getHtml = println[1].split('&');
      var data= {};
      var DataStr = "";
      var thisInput = "";
      for(var i=0;i<getHtml.length;i++)
      {
        thisInput = getHtml[i].split('=');
        data[thisInput[0]] = thisInput[1];
      }
...
function load_init()
{
   ...
   jQuery('#navMenu > a').click(function(){
      ...
      if(block == "selected" || jQuery('#' + block + '_item').text() == "" && block != "query")
      {
         ...
         if(block == "selected")
            url += '&TO_ID_STR=' + to_id_field.value;
   ...
   jQuery('#keyword').keyup(function(event){
      ...
      var kw = this.value.replace(/'/g, "");
      kw = kw.replace(/\?/g, "");
      var url = item_url + '?ACTION=query&' + query_string + '&KEYWORD=' + kw;
      if(jQuery(this).attr('notlogin_flag')=='1')
      {
         url += '&NOTLOGIN_FLAG=1';
      }
