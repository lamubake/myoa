...
function turnUserWorkFlow(control_type){
    ...
    var POST_STR = "RUN_ID="+q_run_id+"&FLOW_ID="+q_flow_id+"&PRCS_ID="+q_prcs_id+"&FLOW_PRCS="+q_flow_prcs+"&PRCS_KEY_ID="+q_prcs_key_id+"&PRCS_ID_NEXT="+prcs_id_next+"&FREE_STEP="+free_step+"&VIEW_USER_ID="+VIEW_USER_ID+"&PRCS_BACK="+prcs_back+"&RX_PRCS="+rx_prcs+"&DING="+DING+plugin_str;
    if(free_step != "" && typeof(free_step) != undefined){
...
function savePublicWorkFlow(){
    ...
    $("#UPLOAD_AREA_PUBLIC .pda_attach").each(function(){
        ...
        var attach_id = $this.attr('data-attach_id');
        var attach_name = $this.attr('data-attach_name');
        public_attach.attach_id += attach_id;
        public_attach.attach_name += attach_name+"*";
    });
                ...
                if(key!=_tmp_key){
                    ...
                    if(/^[A-Za-z0-9]+$/.test(key)){
                        ...
                        $.ajax({
                            ...
                            url: url,
                            cache: false,
                            data: {"KWORD":key, "ACTION": "getNameIndex"},
                            beforeSend: function(){
                                $.ProLoading.show();   
                            },
                            success: function(data)
