...
//一键催办
function onekey_reminders(type)
{
    ...
    if(rowData.length)
    {
        ...
        {
            var OPERATION_STR = jQuery('#gridTable').jqGrid('getCell',rowData[i],'opration');
            var prcs_flag_str = jQuery('#gridTable').jqGrid('getCell',rowData[i],'prcs_flag');
            if(OPERATION_STR.indexOf('reminder_one_key') == -1) //没有催办权限
                continue;
            var OPERATION_S = OPERATION_STR.substring(OPERATION_STR.indexOf('{'),OPERATION_STR.indexOf('}'));
            var operation_string = OPERATION_S.split(",");
            run_id += operation_string[0].replace(/[^0-9]/ig,"")+",";
            prcs_id += operation_string[2].replace(/[^0-9]/ig,"")+",";
            flow_id += operation_string[1].replace(/[^0-9]/ig,"")+",";
			if(prcs_flag_str && prcs_flag_str != "")
				prcs_flag += prcs_flag_str.replace(/<[^>]+>/g,"")+",";//去掉所有的html标记
        }
        reminders(run_id,flow_id,"",prcs_id,1,prcs_flag,type);
    }else
    {
        alert("请至少选择一条记录");
    }
...
function reminders_do(type){
	...
	jQuery.get("/module/workflow/engine/flow_reminders.php",{"RUN_ID":run_id,"FLOW_ID":flow_id,"PRCS_ID":prcs_id,"CONTENT":content,"mobile_check":mobile_check,"prcs_flag":prcs_flag,"type":type},function(data)
	{
