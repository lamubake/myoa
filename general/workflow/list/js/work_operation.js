...
function check_form(){
	...
	switch(checkType){
		case "intrust": //委托
			try{
				...
				jQuery("input[type='text'][name^='intrust_run_']").each(function(){ //验证
					...
					jQuery.ajax({
						type: "POST",
						url: "data/work_to_intrust.php",
						cache: false,
						async: false,
						data: "run_id="+run_id+"&prcs_key_id="+prcs_key_id+"&prcs_id="+prcs_id+"&flow_prcs="+flow_prcs+"&to_user="+to_user,
						error: function(msg){
							alert(msg);
						},
                ...
                */
				...
				if(jQuery('#intrustData').find('.result_error').length > 0){//有错误提示则不提交
					...
					jQuery("input[type='text'][name^='intrust_run_']").each(function(){
						...
						jQuery.ajax({
							type: "POST",
							url: "data/work_to_intrust.php?ispirit_export=1",
							cache: false,
							async: false,
		...
		case "pending"://挂起
			...
			var url="/module/workflow/engine/pending_work.php?PRCS_KEY_ID_STR="+prcs_key_id_str+"&active_time="+active_time;
		...
		case "comment"://批注
			try{
				...
				var url="/module/workflow/engine/set_comment.php?run_id_str="+run_id_str+"&content="+content;
		        jQuery.get(url,{},function(data){
					window.refreshGrid();
		        	//jQuery("#gridTable").jqGrid('setGridParam', { serializeGridData : function(e){ e.connstatus = 1;  return e;} }).trigger('reloadGrid');
		        	jQuery('.close').click();
		...
		case "export"://代办工作导出
			try{
				var url="data/work_to_export.php?ispirit_export=1";
		...
		case "export_instrust"://工作委托导出
			try{
				var url="data/work_to_export_instrust.php?ispirit_export=1";
				jQuery("#form1").attr("action",url);
				jQuery("#form1").submit();
			}catch(e){
				return false;
		...
		case "export_concern"://关注工作导出
			try{
				var url="data/work_to_export_concern.php?ispirit_export=1";
				jQuery("#form1").attr("action",url);
				jQuery("#form1").submit();
			}catch(e){
				alert(e);
		...
		case "export_pending"://挂起工作导出
			try{
				var url="data/work_to_export_pending.php?ispirit_export=1";
				jQuery("#form1").attr("action",url);
				jQuery("#form1").submit();
			}catch(e){
				alert(e);
		...
		case "export_settles"://办结工作导出
			try{
				var url="data/work_to_export_settles.php?ispirit_export=1";
				jQuery("#form1").attr("action",url);
				jQuery("#form1").submit();
			}catch(e){
				alert(e);
		...
		case "export_data_all"://全部工作导出
			try{
				var url="data/work_to_export_data_all.php?ispirit_export=1";
				jQuery("#form1").attr("action",url);
				jQuery("#form1").submit();
			}catch(e){
				return false;
...
{	
	...
	jQuery.ajax({
        ...
        data:{"run_name":run_name,"flow_name":flow_name,"outoftime":outoftime,"Priority":Priority,"flow_id_check":flow_id_check,"outoftime_check":outoftime_check,"Priority_check":Priority_check,"search_para":search_para},
...
{	
	...
	var flow_name = jQuery(window.frames["workflow-data-list"].document).find("#flow_id").find("option:selected").text();
	var flow_id_check = jQuery(window.frames["workflow-data-list"].document).find("#flow_id").val();
	//var intrust_man = jQuery(window.frames["workflow-data-list"].document).find("#intrust_man").val();
	var search_para = jQuery(window.frames["workflow-data-list"].document).find("#search_para").val();
	var flag_search=jQuery(window.frames["workflow-data-list"].document).find("#search_adv_div").css("display");
	if(flag_search=='none'){
		run_name='';
...
{	
	...
	jQuery.ajax({
        ...
        data:{"run_id":run_id,"run_name":run_name,"flow_name":flow_name,"flow_id_check":flow_id_check,"prcs_flag":prcs_flag,"search_para":search_para},
        success:function(html){
            jQuery('.modal-body').append(html);
			jQuery.getJSON('/general/workflow/get_flow_list.inc.php', {root:true,action:3}, function(jsonData){
...
function work_run_restore_work() 
{
	if(checkSelectRows()){
		...
		var url="/module/workflow/engine/re_pause_work.php?run_id_str="+run_id_str+"&prcs_key_id_str="+prcs_key_id_str;
	    jQuery.get(url,{},function(data){
	    	window.refreshGrid();
	    	//jQuery('.close').click();
	    });	
...
function work_run_cancel_concern() 
{
	...
	if(checkSelectRows()){
	    ...
	    {
	        jQuery.get("/module/workflow/engine/focus_work.php",{"RUN_ID":run_id_str,"OP":OP,"FLOW_ID":flow_id_str},function(data)
	        {
	    	    window.refreshGrid();
...
{
	var url = "/general/workflow/new/index.php";
