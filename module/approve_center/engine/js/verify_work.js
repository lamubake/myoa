...
function comment(RUN_ID,FLOW_ID,BEGIN_DEPT,ARCHIVE_ID){
    ...
    window.open("/module/approve_center/engine/verify_work.php?RUN_ID="+RUN_ID+"&FLOW_ID="+FLOW_ID+"&BEGIN_DEPT="+BEGIN_DEPT+ARCHIVE_ID_STR,"comment","status=0,toolbar=no,menubar=no,width=550,height=200,location=no,scrollbars=yes,resizable=no,left="+myleft+",top="+mytop);
}
...
function delete_run(RUN_ID,FLOW_ID,BEGIN_DEPT)
{
    ...
    if(window.confirm(msg))
    {
        jQuery.get("/module/approve_center/engine/delete_work.php",{"action":"delete_work","run_id_str":RUN_ID,"flow_id":FLOW_ID,"begin_dept":BEGIN_DEPT},function(data)
        {
...
//工作催办
function reminders(run_id,flow_id,run_name,user_id,key_prcs_id)
{
...
function reminders_do(){
    ...
    jQuery.get("/module/approve_center/engine/flow_reminders.php",{"RUN_ID":run_id,"FLOW_ID":flow_id,"USER_ID":user_id,"KEY_PRCS_ID":key_prcs_id,"CONTENT":content},function(data)
    {
